...
async function handleFirstMonthBonus(
  id: string,
  purchasePayload: any,
  senderId: string,
  retry: boolean
...
async function deliverBonus(email: string, number: string, networkID: number, senderId: string) {
  ...
  try {
    if (process.env.NODE_ENV !== 'production')
      return sendMessage(senderId, { text: 'Evironment does not support monthly bonus delivery.' });
    ...
    if (response && response.data.Status === 'successful') {
      ...
      if (senderId) {
        ...
        await sendMessage(senderId, {
          text: `you've recieved ${bonus.size} on your ${number} ${bonus.network} line for your first transaction of the month...`,
        });
      }
      return;
    }
