...
import {
  getCurrentNumberOfWinnersFB,
  totalAcceptableWinnersFB,
...
const free3gbParticipationReminderW = async (user: BotUserType) => {
  try {
    ...
    const numberOfWinners = getCurrentNumberOfWinnersFB();
    ...
    if (
      numberOfWinners < totalAcceptableWinnersFB &&
...
const handleNewMonthNotification = async (
  ...
  const message = wonLastMonth
    ? currentWinners < totalAcceptableWinnersFB
      ...
      : `You won last month but missed this month. ${totalAcceptableWinnersFB} users already qualified. Try next month!`
    : currentWinners < totalAcceptableWinnersFB
...
const handlePoolAvailableNotification = async (user: BotUserType, transactionCount: number) => {
  await sendMessageW(
    user.id,
    `Only ${totalAcceptableWinnersFB - getCurrentNumberOfWinnersFB()} spots left! Make additonal ${
      3 - transactionCount
