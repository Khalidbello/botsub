...
import { validateUserTransactionMonth } from './concluded_transaction_propmpter_fb';
import { getCurrentNumberOfWinners, totalAcceptableWinners } from './number_of_winners_logic_fb';
...
const free3gbParticipationReminderFB = async (user: BotUserType) => {
  try {
...
const handleNewMonthNotification = async (
  ...
  if (message) {
    await sendMessage(user.id, { text: message });
  }
...
const handleWinnerReminder = async (user: BotUserType, currentDate: Date) => {
  ...
  if (!hasClaimed) {
    await sendMessage(user.id, { text: "Don't forget to claim your free 3GB! Enter C to claim." });
  }
};
...
const handleMissedOpportunityNotification = async (user: BotUserType) => {
  await sendMessage(user.id, {
    text: "You missed this month's free 3GB offer. Action takers got rewarded!. \n\nYou get another chance next month, be among the action takers.",
  });
...
export { free3gbParticipationReminderFB, BotUserType };
