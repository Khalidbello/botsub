...
const sendMessage = require('./send_message.js'); const { sendNewConversationResponse, cancelTransaction } = require('./postback_responses.js');
const {
...
async function processMessage(event, res) {
  ...
  switch (user.nextAction) {
    ...
    case 'buyData':
      handleBuyData(event);
      break;
    case 'selectDataNetwork':
      handleDataNetWorkSelected(event);
      break;
    case '':
